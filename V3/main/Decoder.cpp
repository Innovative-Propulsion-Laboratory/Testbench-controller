    ...
    */
    if (instructions[0] == 0xAA && instructions[1] == 0xAA && instructions[2] == 0xAA && instructions[3] == 0xAA) {  // Start test
      if (assembleUInt16(instructions[5], instructions[4]) == 0) {
        ...
        Sequence_data.Igniter_pressure_timemax = assembleUInt16(instructions[47], instructions[46]);
        ...
        */
        Sequence_data.ETH_to_LOX_bypass = assembleUInt16(instructions[53], instructions[52]);
        Serial.print("ETH to LOX bypass : ");
        Serial.println(Sequence_data.ETH_to_LOX_bypass);

        Sequence_data.Bypass_check_duration = assembleUInt16(instructions[55], instructions[54]);
        Serial.print("Bypass check duration : ");
        Serial.println(Sequence_data.Bypass_check_duration);

        Sequence_data.Bypass_pressure = assembleUInt16(instructions[57], instructions[56]);
        Serial.print("Bypass pressure : ");
        Serial.println(Sequence_data.Bypass_pressure);

        Sequence_data.Bypass_verified_duration = assembleUInt16(instructions[59], instructions[58]);
        Serial.print("Bypass verified duration : ");
        Serial.println(Sequence_data.Bypass_verified_duration);

        Sequence_data.GOX_to_ETH = assembleUInt16(instructions[61], instructions[60]);
        Serial.print("GOX to ETH : ");
        Serial.println(Sequence_data.GOX_to_ETH);

        Sequence_data.ETH_to_LOX_main = assembleUInt16(instructions[63], instructions[62]);
        Serial.print("ETH to LOX main : ");
        Serial.println(Sequence_data.ETH_to_LOX_main);

        Sequence_data.Main_check_duration = assembleUInt16(instructions[65], instructions[64]);
        Serial.print("Main check duration : ");
        Serial.println(Sequence_data.Main_check_duration);

        Sequence_data.Main_pressure = assembleUInt16(instructions[67], instructions[66]);
        Serial.print("Main pressure : ");
        Serial.println(Sequence_data.Main_pressure);

        Sequence_data.Main_verified_duration = assembleUInt16(instructions[69], instructions[68]);
        Serial.print("Main verified duration : ");
        Serial.println(Sequence_data.Main_verified_duration);

        Sequence_data.TVC_pattern_duration = assembleUInt16(instructions[71], instructions[70]);
        Serial.print("TVC pattern duration : ");
        Serial.println(Sequence_data.TVC_pattern_duration);

        Sequence_data.LOX_to_ETH_closing_delay = assembleUInt16(instructions[73], instructions[72]);
        Serial.print("LOX to ETH closing delay : ");
        Serial.println(Sequence_data.LOX_to_ETH_closing_delay);

        Sequence_data.Purge_duration3 = assembleUInt16(instructions[75], instructions[74]);
        Serial.print("Purge duration 3 : ");
        Serial.println(Sequence_data.Purge_duration3);

        Sequence_data.Cooling_duration_after_end_burn = assembleUInt16(instructions[77], instructions[76]);
        ...
        // ETH pressure bangbang set
        uint16_t value;
        int idx = 6;  // <<< départ à 6
        ...
        // Purge duration 1
        Sequence_data.Purge_duration1 = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("Purge duration 1 : ");
        Serial.println(Sequence_data.Purge_duration1);

        // Courant limite (GP_current)
        Sequence_data.GP_current = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("Courant limite : ");
        Serial.println(Sequence_data.GP_current);

        // Temps de chauffe (Glowplug_heat_before_duration)
        Sequence_data.Glowplug_heat_before_duration = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("Temps de chauffe : ");
        Serial.println(Sequence_data.Glowplug_heat_before_duration);

        // Current_raising
        Sequence_data.Current_raising = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("Current_raising : ");
        Serial.println(Sequence_data.Current_raising);

        // ETH to GOX
        Sequence_data.ETH_to_GOX = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("ETH to GOX : ");
        Serial.println(Sequence_data.ETH_to_GOX);

        // Igniter chamber pressure
        Sequence_data.Igniter_chamber_pressure = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("Igniter chamber pressure : ");
        Serial.println(Sequence_data.Igniter_chamber_pressure);

        // Igniter pressure time max
        Sequence_data.Igniter_pressure_timemax = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("Igniter pressure time : ");
        Serial.println(Sequence_data.Igniter_pressure_timemax);

        // Igniter high pressure time
        Sequence_data.Igniter_Highpressure_time = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("Igniter Highpressure time : ");
        Serial.println(Sequence_data.Igniter_Highpressure_time);

        // Igniter burn duration
        Sequence_data.Igniter_burn_duration = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("Igniter burn duration : ");
        Serial.println(Sequence_data.Igniter_burn_duration);

        // GOX to ETH
        Sequence_data.GOX_to_ETH = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("GOX to ETH : ");
        Serial.println(Sequence_data.GOX_to_ETH);

        // Purge duration 3
        Sequence_data.Purge_duration3 = assembleUInt16(instructions[idx + 1], instructions[idx]);
        idx += 2;
        Serial.print("Purge duration 3 : ");
        Serial.println(Sequence_data.Purge_duration3);
