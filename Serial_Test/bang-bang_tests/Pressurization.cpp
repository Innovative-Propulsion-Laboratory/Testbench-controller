...
void BB_param_set(uint8_t tank, uint16_t pressure){ // tank: 1 = LOX, 2 = ETH, 6 = WATER
    if (tank == 1){
...
void BB_enable (uint8_t tank, bool command){ // tank: 1 = LOX, 2 = ETH, 6 = WATER
    if (command == 0){
        if (tank == 1) {LOX_BB = 0;}
        if (tank == 2) {ETH_BB = 0;}
...
void BB_pressurization(uint16_t PS11, uint16_t PS21, uint16_t PS61, uint16_t PS62){
    ...
    if (WATER_BB){
        if ((PS61 <= WATER_BB_min) || (PS62 <= WATER_BB_min)) {setValve(SV52, 1); setValve(SV53, 1);}
        if ((PS61 >= WATER_BB_max) || (PS62 >= WATER_BB_max)) {setValve(SV52, 0); setValve(SV53, 0);}
    }
}